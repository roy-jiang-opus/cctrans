...
- 🔄 **추가 또는 대체** — 번역을 영어 아래에 표시하거나, `cctrans display replace`로 그 자리에 번역만 표시
- ❓ **질문 대화상자 번역** — Claude Code의 대화형 질문 프롬프트도 자신의 언어로 표시되며, 모델은 여전히 영어 답변을 읽습니다
...
- 🎚️ **대화형 설정** — `cctrans settings`가 단일 화면 편집기를 엽니다(방향키 조작). 간격, 마커, 모델 등을 조정, 기본 + 고급
...
설치가 훅을 등록하고 대화형 설정 편집기를 엽니다(언어, 표시 모드, 백엔드, API 키 — `cctrans settings`로 언제든 다시 열 수 있습니다). 그다음 **Claude Code를 재시작** — 응답이 이중 언어가 됩니다. Claude Code 입력창에 `!cctrans off` / `!cctrans on`을 입력해 언제든 전환(`!`는 CC 내장 bash 모드 — 모델 호출 없음, 토큰 없음).
...
| `cctrans display [append\|replace]` | 번역을 영어 아래에 표시하거나, 영어 대신 표시(line 모드) |
| `cctrans only [on\|off]` | 번역**만** 표시하고 영어를 숨김(line 모드 + replace) |
| `cctrans dialog [on\|off]` | Claude Code의 질문 대화상자 번역(기본값 켜짐) |
| `cctrans settings` | 대화형 설정 편집기 열기(기본 + 고급) |
...
**추가 또는 대체.** 기본적으로 번역은 영어 *아래에* 표시됩니다(이중 언어). 자신의 언어만 읽고 싶으신가요? `cctrans display replace`를 사용하면 각 영어 줄 **대신** 번역이 표시됩니다:

```bash
cctrans display replace   # 번역만 · cctrans display append — 이중 언어로 복귀
```

대체는 **line 모드**에서 적용됩니다(section/message는 설계상 영어를 먼저 스트리밍하므로 대체할 대상이 없습니다). 어느 쪽이든 트랜스크립트와 모델의 컨텍스트는 100% 영어로 유지됩니다; 번역할 수 없는 줄은 원본 텍스트를 유지하므로 사라지는 것은 없습니다.

**영어는 전혀 없이 자신의 언어만 원하시나요?** `cctrans only on`이 바로 그것을 위한 한 줄 명령어 단축키입니다(line 모드 + replace를 설정합니다). 영어는 한순간도 비치지 않습니다 — Claude Code가 각 줄을 이미 번역된 상태로 렌더링하기 때문입니다 — 그리고 번역할 수 없는 줄은 해당 영어로 폴백됩니다; 코드 블록은 그대로 통과합니다. `cctrans only off`는 이중 언어로 되돌립니다.

## ❓ 질문 대화상자

Claude Code가 선택지에서 고르라고 요청할 때(대화형 질문 대화상자), 질문, 선택지 라벨, 그 설명도 자신의 언어로 표시됩니다 — 그리고 선택한 답변은 여전히 **영어**로 모델에 전달되므로 모델의 추론은 영어로만 유지됩니다:
...
이는 Claude Code의 `PreToolUse`/`PostToolUse` 훅에 올라타 동작합니다(질문 대화상자는 도구 입력에서 렌더링되며, 메시지 오버레이로는 닿을 수 없기 때문입니다). 자신의 `display` 설정을 따릅니다 — append 모드에서는 이중 언어, replace 모드에서는 자신의 언어만. 기본값 켜짐; `cctrans dialog off`로 끌 수 있습니다. 대화상자를 제때 번역할 수 없으면 영어로 그대로 표시됩니다.

> **업그레이드?** 업데이트 후 한 번 `cctrans install`을 실행하여 새 대화상자 훅을 등록하세요. 답변 복원(모델의 답변을 영어로 유지)에는 Claude Code ≥ 2.1.121이 필요합니다 — `cctrans doctor`가 이전 버전에서 경고합니다.
