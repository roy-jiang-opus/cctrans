...
Claude Code에 **이중 언어 오버레이**를 추가합니다: 모든 응답에서 원본 영어 줄 아래에 번역(중/일/한/러)이 한 줄씩, **대화 안에 그대로** 표시됩니다.
...
cd cctranslate
node bin/tt.js install      # 훅을 ~/.claude/settings.json에 등록하고 tt를 ~/.local/bin에 링크
```
...
| `tt lang [code]` | 대상 언어 표시/설정: `zh-CN` `zh-TW` `ja` `ko` `ru` |
...
| 백엔드 | 요구사항 | 속도 | 품질 | 비고 |
|--------|----------|------|------|------|
| `openai` (키 있으면 기본) | `OPENAI_API_KEY` | ~1.4s/조각 | 높음 | `gpt-4o-mini` 줄 배치 번역, 코드/경로 보존 |
| `anthropic` | `ANTHROPIC_API_KEY` | ~1s/조각 | 높음 | `claude-haiku-4-5` + structured outputs, 엄격한 등길이 줄 배열 (~$0.0005/조각) |
| `deepl` | `DEEPL_API_KEY` (무료 50만 자/월) | ~0.5s/조각 | 높음 | 전통 MT 최고 품질; 배열 API로 줄이 자연히 정렬 |
| `azure` | `AZURE_TRANSLATOR_KEY` (무료 200만 자/월) | ~0.5s/조각 | 중상 | `AZURE_TRANSLATOR_REGION` 추가 가능 |
...
환경 변수: `TT_BACKEND`, `TT_TARGET` (기본 `zh-CN`), `TT_MARKER` (기본 `↳ `), `TT_HOME` (기본 `~/.cc-translate`), `TT_OPENAI_MODEL`, `TT_ANTHROPIC_MODEL`, `AZURE_TRANSLATOR_ENDPOINT`.

## 다국어

대상 언어는 **CJK + 러시아어**를 지원합니다 (비라틴 문자이므로 유니코드 범위로 "이 줄은 이미 대상 언어"를 무비용 판별하여 건너뜁니다):

```bash
tt lang ja      # 일본어
tt lang ko      # 한국어
tt lang ru      # 러시아어
tt lang zh-TW   # 번체 중국어
tt lang zh-CN   # 간체 중국어 (기본)
```

언어 전환은 즉시 적용됩니다 (훅이 호출마다 상태를 읽음); 언어별 캐시는 독립적입니다.
